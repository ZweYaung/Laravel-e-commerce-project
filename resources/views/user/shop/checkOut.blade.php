                        ...
                        <form action="{{ route('user#order') }}" method="post" enctype="multipart/form-data">
                                    ...
                                    <select name="paymentType"
                                        ...
                                        @foreach ($paymentAcc as $item)
                                            <option value="{{ $item->id }}"
                                                @if (old('paymentType') == $item->id) selected @endif>{{ $item->type }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('paymentType')
                            ...
                            <div class="row mt-4">
                                ...
                                <div class="col">
                                    ...
                                    Total : <span class="fw-bold">{{ number_format($orderTemp[0]['finalAmt']) }} MMK</span>
                                </div>
                            </div>
                            <div class="row mt-4 mx-2">
                                <button type="submit" class="btn btn-outline-secondary rounded w-100">
                                    <i class="fa-solid fa-cart-shopping me-3"></i>Place Order
                                </button>
                            </div>
                        </form>
                    </div>
